...
def on_keyboard_event(event):
    if event.Key == togglekey.get().upper():
        toggle_clicking()
        return False
    return True
def start_hook():
...
frame.grid()
ttk.Label(frame, text="DodyClicker 1.1").grid(row=0, column=0, sticky="w")
...
key_display = tk.Label(frame, text=togglekey.get(), width=4, height=2,relief="groove", borderwidth=2, anchor="center", font=("Arial", 12))
