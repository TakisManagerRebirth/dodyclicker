...
def setkey():
    ...
    "BACKSPACE":0x08,"TAB":0x09,"CLEAR":0x0C,"ENTER":0x0D,"SHIFT":0x10,"CTRL":0x11,"ALT":0x12,"PAUSE":0x13,"CAPSLOCK":0x14,
    "ESC":0x1B,"SPACE":0x20,"PAGEUP":0x21,"PAGEDOWN":0x22,"END":0x23,"HOME":0x24,
    "LEFT":0x25,"UP":0x26,"RIGHT":0x27,"DOWN":0x28,"SELECT":0x29,"PRINT":0x2A,"EXECUTE":0x2B,"PRINTSCREEN":0x2C,"INS":0x2D,"DEL":0x2E,"HELP":0x2F,
...
ttk.Label(f,text="DodyClicker 1.4",font=(F,int(12*W))).grid(row=0,column=0,sticky="w")
ttk.Label(f,text="Click wait (seconds):",font=(F,int(10*W))).grid(row=1,column=0,sticky="w")
ttk.Entry(f,textvariable=iv,width=8,font=(F,int(10*W))).grid(row=1,column=1,sticky="w")
ttk.Label(f,text="Trigger key:",font=(F,int(10*W))).grid(row=2,column=0,sticky="w")
ttk.Entry(f,textvariable=tkv,width=8,font=(F,int(10*W))).grid(row=2,column=1,sticky="w")
ttk.Button(f,text="Apply Key",command=setkey).grid(row=2,column=2,padx=6)
...
btn=ttk.Button(f,text="Start",command=toggle,width=12,padding=(6,4))
btn.grid(row=5,column=1,pady=int(8*W),sticky="w")
kd_lbl=tk.Label(f,textvariable=kd,width=4,height=2,relief="groove",borderwidth=2,anchor="center",font=(F,int(12*W)))
kd_lbl.grid(row=2,column=3,padx=8)
